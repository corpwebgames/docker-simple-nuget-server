...
RUN sed -i -- 's/.*#gzip  on;/    client_max_body_size 400M;/g' /etc/nginx/nginx.conf && \
    cat /etc/nginx/nginx.conf 


# sed -i -- 's/fastcgi_pass php/fastcgi_pass unix:\/run\/php\/php7.0-fpm.sock/' /etc/nginx/conf.d/default.conf && \
RUN sed -i -- 's/.*upload_max_filesize.*=.*/upload_max_filesize = 400M/g' /etc/php/7.0/fpm/php.ini && \
